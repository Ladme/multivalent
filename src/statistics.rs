...
impl MoveStatistics {
    ...
    pub fn report(&self) {
        ...
        let total_chain = self.chain_accepted + self.chain_rejected;

        println!("\nMove Statistics for Chain Moves:");
        println!(">> Total chain moves: {}", total_chain);
        println!(">> Accepted chain moves: {} ({:.2} %) ", self.chain_accepted, (100.0 * self.chain_accepted as f64 / total_chain as f64));
        println!(">> Rejected chain moves: {} ({:.2} %) ", self.chain_rejected, (100.0 * self.chain_rejected as f64 / total_chain as f64));
